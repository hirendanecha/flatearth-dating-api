...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.Username;
  let msg = `You have been assign in FaltEarthDating channel by the FaltEarthDating Admin.
  To access your channel, log into your FaltEarthDating account,click on the
  FaltEarthDating icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    if (isApprove === "Y") {
      ...
      msg = `FaltEarthDating has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
